...
g.space = get.gene.coexpression.space(objE17, 
                                      n.genes.for.marker = 28,  primary.markers = unlist(layers))
```
```{r}
...
selectTOM = dissTOM[unique(unlist(layers)), unique(unlist(layers))];
# There’s no simple way of restricting a clustering tree to a subset of genes, so we must re-cluster.
selectTree = hclust(as.dist(selectTOM), method = "average")
moduleColors = mergedColors
names(moduleColors) = rownames(dissTOM)
selectColors = moduleColors[unique(unlist(layers))]
