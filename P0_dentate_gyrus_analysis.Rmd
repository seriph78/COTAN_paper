...
```
```{r}
data = as.data.frame(fread(paste(data_dir,"separation_age/P0.csv", sep = "/")))
...
obj = initRaw(obj,GEO="GSE104323" ,sc.method="10X",cond = "mouse dentate gyrus P0")
```
...
```{r eval=FALSE, include=TRUE}
obj2 = automatic.COTAN.object.creation(df = data,GEO="GSE104323" ,sc.method="10X",cond = "P0_dent_gyrus",
                                       out_dir =  "Data/")
```
```{r include=FALSE}
obj2 = readRDS(paste(out_dir,"P0_dent_gyrus",".cotan.RDS", sep = ""))
```
...
plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "P0_dent_cleaned",dir = "Data/")
```
## Heatmaps without cleaning
```{r}
list.genes = list("Ref.col"= PNGs, "NPGs"=NPGs, "Const."=hk )

plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "P0_dent_gyrus",dir = "Data/")
```
