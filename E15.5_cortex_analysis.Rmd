...
```{r}
data = as.data.frame(fread(paste(data_dir,"GSE107122_E155_Combined_Only_Cortical_Cells_DGE.txt.gz", sep = "/")))
...
obj = initRaw(obj,GEO="GSE107122" ,sc.method="DropSeq",cond = "mouse cortex E15.5")
```
...
```{r}
yset = 0.28#threshold to remove low UDE cells
...
```{r eval=FALSE, include=TRUE}
obj2 = automatic.COTAN.object.creation(df = data,GEO="GSE107122" ,sc.method="Drop-seq",cond = "E15_cortex",
                                       out_dir =  "Data/")
```
...
```{r include=FALSE}
obj2 = readRDS(paste(out_dir,"E15_cortex",".cotan.RDS", sep = ""))
```
```{r}
obj@meta
```
...
plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "E15_cortex_clean",dir = "Data/")
```
...
plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "E15_cortex",dir = "Data/")
```
