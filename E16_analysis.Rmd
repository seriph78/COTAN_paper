---
title: "E16.5 mouse dentate gyrus"
output:
  html_document:
    collapsed: no
    css: html-md-01.css
...
```{r}
data_dir = "Data/mouse_dentate_gyrus/"
```
```{r}
data = as.data.frame(fread(paste(data_dir,"separation_age/E16.5.csv", sep = "/")))
...
obj = initRaw(obj,GEO="GSE104323" ,sc.method="10X",cond = "mouse dentate gyrus E16.5")
```
...
```{r}
yset = 0.61#threshold to remove low UDE cells
plot.ude <- ggplot(nu_df, aes(x = n, y=nu)) + 
    geom_point(colour = "#8491B4B2", size=1) + 
    my_theme + ylim(0.5,1) + xlim(0,800) +
...
obj2 = automatic.COTAN.object.creation(df = data,GEO="GSE104323" ,sc.method="10X",cond = "E16_dent_gy",
                                       out_dir =  "Data/")
```
...
obj2 = readRDS(paste(out_dir,"E16_dent_gy",".cotan.RDS", sep = ""))
```
```{r}
obj@meta
```
...
plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "E16.5_cleaned",dir = "Data/")
```
...
plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "E16_dent_gy",dir = "Data/")
```
